 ...
 */
static int
col_rel_cow_unshare_raw(col_rel_t *r, uint32_t new_cap)
{
    if (!r || !r->col_shared)
        return 0;
    if (new_cap == 0)
...
{
    bool was_shared = r && r->col_shared != NULL;
    int rc = col_rel_cow_unshare_raw(r, new_cap);
    if (rc == 0 && was_shared)
...
col_rel_append_row(col_rel_t *r, const int64_t *row)
{
    ...
    if (needs_resize) {
        ...
        /* Complete the new row in private storage before publication.  No
         * operation below this point can fail, so the relation remains
         * unchanged until the commit point. */
        for (uint32_t c = 0; c < r->ncols; c++) {
            int64_t value = row[c];
            if (r->column_types && r->column_types[c] == WIRELOG_TYPE_FLOAT
                && wl_columnar_float_bits_zero(value))
                value = 0;
            new_cols[c][r->nrows] = value;
        }
        if (new_ts)
            memset(&new_ts[r->nrows], 0, sizeof(*new_ts));
        col_rel_publish_resize(r, new_cols, new_ts, new_cap);
        col_rel_ledger_reconcile(r, ledger_before);
        wl_columnar_relation_touch_storage(r);
    } else {
        /* Spare capacity still does not make a shared view writable. */
        if (r->col_shared) {
...
col_rel_append_all(col_rel_t *dst, const col_rel_t *src, wl_arena_t *arena)
{
    ...
    if (new_nrows > dst->capacity) {
        ...
        int64_t **new_cols = NULL;
        col_delta_timestamp_t *new_ts = NULL;
        int prepare_rc = col_rel_prepare_resize(dst, new_cap, &new_cols,
                &new_ts);
        if (prepare_rc != 0) {
            free(prepared_types);
            if (prepared_schema_ok)
    ...
    } else if (dst->col_shared) {
        /* The no-resize path must detach before bulk memcpy. */
...
col_rel_radix_sort(col_rel_t *r, uint32_t start_row, uint32_t nrows)
{
    ...
    if (borrowed) {
        ...
        if (col_rel_cow_unshare_raw(r, 0) != 0) {
            free((void *)old_columns);
            free(old_shared);
            return ENOMEM;
        }
